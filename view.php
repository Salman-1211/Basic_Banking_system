<?php 
include ('connect.php');

if(isset($_GET['id'])) 
{   
    //Session Start
	session_start();	
	$_SESSION['id'] = $_GET['id'];
}
	
	


	if(isset($_GET['errors'])){
		$error=$_GET['errors'];
		echo "<div class='alert alert-danger'>
            $error</div>";
			
	}
	if(isset($_GET['success'])){
		$success= $_GET['success'];
		echo "<div class='alert alert-success'>
           $success</div>";
	}

		$id = $_GET['id'];
		$qry = "SELECT * FROM user where id = '$id'";
		$result = mysqli_query($con, $qry);



?>

<html>
<head>
<title>Transfer Credit</title>
<style>

body{
        background: url('images/img2.gif') no-repeat center center fixed; 
  -webkit-background-size: cover;
  -moz-background-size: cover;
  -o-background-size: cover;
  background-size: cover;
  width: 100%
  height: auto;
		
}


#my_table1{
		margin-left:60px;
		
		font-size: 19px;
		width: 480px;
		border-collapse: collapse;
		text-align: center;
	}
	td{
		padding: 9px;
		font-size: 18px;
		background-color: #fff;
	}
	th{
		padding: 8px;
		font-size: 20px;
	}
h1
{
color: #FFD700;

}
h2
{
color: #FFD700;

}
select{

color: #FFD700;
}
footer{
        bottom: 0;
		width: 100%;
        position: fixed;        
        background: #333;
        padding: 10px 0;
        color: #fff;
		margin-left: -8px;
		height: 20px;
    }
	.text-white{
	color:white;
}

.center {
    text-align: center;
    
    color: black;
    width: auto;
    height: 50px;
}


.container { 
  height: 200px;
  position: relative;
color:black;
  
}

.center1 {
  margin: 0;
  position: absolute;
  top: 50%;
  left: 50%;
  -ms-transform: translate(-50%, -50%);
  transform: translate(-50%, -50%);
}

.center2 {
  display: flex;
  justify-content: center;
  align-items: center;

  
}
</style>
<meta name="viewport" content="width=device-width, initial-scale=1">
<link type="text/css" rel="stylesheet" href="css/view1.css" >
</head>

<body>
	
	
<div class="container1">
	<p align='right'>
		<br>
		<br>
		<br>
		<br>
		<br>
	<button class='button1' onclick="redirect()">Back</button>
	</p>
<script>
	function redirect() {
		window.location.href = "user.php";
	}
</script>
	
	
<form method="POST" action="transfercredit.php">
	
	<div class="head">
		<h1 style="text-align:center;">Transfer Credit</h1>
		<h2 style="text-align:center;">From:</h2>
		<table id=\"my_table1\" border='1' style="margin-left:550px; margin-top:-380px; color: black;">
				<tr>
				<th>Id</th>
				<th>Name</th>
				<th>Email</th>
				<th>Current Balance</th>
				</tr>
	<?php while($data = mysqli_fetch_assoc($result))
    { ?>
    <tr>
		<td><?php echo $data["id"]; ?></td>
        <td><?php echo $data["name"]; ?></td>
        <td><?php echo $data["email"]; ?></td>
        <td><?php echo $data["credit"]; ?></td>
    </tr>
	
    <?php } ?>
	</div>	
	<h2 style="text-align:center; margin-top: 150px; margin-left: 80px;">Select Name to transfer the Credits:</h2>
		
		<div class="center">
			<select class="form-control" required name="touser" id="listu1">
				<option value="">Select Users</option>
				<?php
				   // var $space = " ";
					$id = $_GET["id"];
                    $qry2 = "SELECT * FROM user where id != '".$id."'";
                    $result2 = mysqli_query($con,$qry2);

                    while($data2 = mysqli_fetch_assoc($result2))
                    {?>
                    <option value="<?php echo $data2['id'];?>"> <?php echo $data2['name']; echo ""; echo " - "; echo $data2['credit'];?>
					
					</option>
					<?php
                    }
				?>
            </select>
		</div>
		
		<div class="center2">

			<h2 style="text-align:center;"><b>Amount</b></h2>
			<br>
			<input type="text" name="credit" placeholder="Enter Amount" required="required">
			<input name="fromtc" type="hidden" value="<?php echo $id;?>">
			<br>
			<button class='button2' name='transfer' onclick="myfunction()">Transfer Credits</button>
</form>	
</div>
			<br><br><br>

			<!--<script>
			function myfunction(){
				alert();
			}
			</script>-->
	<br>
	    
</div>
 <!-- Footer -->
 <footer class="bg-dark text-center text-white" style="text-align: center;">
    <!-- Copyright -->
  <div class="text-center p-3" style="background-color: rgba(0, 0, 0, 0.2);">
    © 2021 Copyright:
    <a class="text-white" href="#">Made By Salman Mulani</a>
  </div>
  <!-- Copyright -->
</footer>
<!-- Footer -->
			  <!-- Optional JavaScript -->
    <!-- jQuery first, then Popper.js, then Bootstrap JS -->
    <script
      src="https://code.jquery.com/jquery-3.3.1.slim.min.js"
      integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo"
      crossorigin="anonymous"
    ></script>
    <script
      src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.7/umd/popper.min.js"
      integrity="sha384-UO2eT0CpHqdSJQ6hJty5KVphtPhzWj9WO1clHTMGa3JDZwrnQq4sF86dIHNDz0W1"
      crossorigin="anonymous"
    ></script>
    <script
      src="https://stackpath.bootstrapcdn.com/bootstrap/4.3.1/js/bootstrap.min.js"
      integrity="sha384-JjSmVgyd0p3pXB1rRibZUAYoIIy6OrQ6VrjIEaFf/nJGzIxFDsf4x0xIM+B07jRM"
      crossorigin="anonymous"
    ></script>

</body>
</html>
